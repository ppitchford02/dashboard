...
let request,desk={picks:[],checks:[],revisions:[],latestRun:null},loaded=false,busy=false,editing=null,current=null,activeSource='',held=[],agentToken=null;
...
function plainLine(p){const selection=String(p.selection||'').trim().replace(/\s+/g,' '),lower=selection.toLowerCase(),event=String(p.event||'').trim().replace(/\s+/g,' ');
  ...
  return selection;}
function currentPick(p, now=Date.now()) {
  if(p.archived || !['pending','review'].includes(p.status)) return false;
  const start=Date.parse(p.eventStartAt);
  return Number.isFinite(start) && start>now && /^https:\/\//.test(p.eventTimeSource||'');
}
function cleanList(items){const lines=[],withheld=[],seen=new Set();
  for(const group of groupPicks(items.filter(p=>currentPick(p)&&p.kind!=='lean'))){
...
const DIRECT_CARD_CUE=/\b(?:add(?:ing)?\s+(?:this|it)?\s*to\s+(?:the|my|our)\s+card|first\s+touchdown\s+scorer(?:\s+lotto)?|anytime\s+touchdown(?:\s+scorer)?|(?:official|final)\s+(?:pick|play)|card\s*(?:is|:)|cheat\s*sheet)\b/i;
...
const normalizeSelection=value=>String(value||'').toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
function classify(text,selection=''){const {candidates,ignored}=extractFromTranscript(text),plain=normalizeSelection(text),chosen=normalizeSelection(selection);
  if(candidates.some(c=>c.kind==='firm'))return{outcome:'firm',reason:''};
  if(candidates.some(c=>c.kind==='lean'))return{outcome:'lean',reason:''};
  if(DIRECT_CARD_CUE.test(text)||(chosen&&plain.includes(chosen)))return{outcome:'firm',reason:''};
...
let recheck=null;
function showClass(){const derived=classify(field('originalText').value,field('selection').value);field('kind').value=derived.outcome==='lean'?'lean':'firm';$('pick-class-state').textContent=CAPTURE_LABEL[derived.outcome]+(derived.outcome==='unclear'?': '+derived.reason:'');return derived;}
...
function renderRunStatus(){
  const host=$('picks-run-status'),run=desk.latestRun;
  host.replaceChildren();
  if(!run){host.className='receipt error';host.textContent='No Sports Picks run has reported yet.';return;}
  const ok=run.outcome==='complete'||run.outcome==='no_work';
  host.className='receipt'+(ok?'':' error');
  const start=Date.parse(run.startedAt),end=Date.parse(run.completedAt);
  const captured=desk.picks.filter(p=>{const at=Date.parse(p.createdAt);return Number.isFinite(start)&&Number.isFinite(end)&&at>=start&&at<=end;});
  const groups=new Map();
  for(const p of captured.filter(p=>currentPick(p))){const name=source(p.sourceId)?.name||p.sourceId;if(!groups.has(name))groups.set(name,[]);groups.get(name).push(p);}
  for(const [name,picks] of groups){
    host.append(el('strong','',name));const list=el('ul','picks-run-list');
    for(const p of picks){const selection=String(p.selection||'Selection not recorded');const price=Number.isInteger(p.odds)?' ('+odds(p.odds)+')':'';const flag=p.status==='review'?' [needs review]':p.kind==='lean'?' [lean]':'';list.append(el('li','',selection+price+flag));}
    host.append(list);
  }
  if(!captured.length)host.append(el('p','',run.picksSaved?'Saved selections could not be matched to this run; see the picks below.':'No new picks saved in this run.'));
  const review=captured.filter(p=>p.status==='review').length;
  host.append(el('p','',`${run.accountsChecked} accounts checked · ${run.picksSaved} picks saved · ${review} need review${run.accountsBlocked?' · '+run.accountsBlocked+' accounts blocked':''}`));
  host.append(el('small','',`${ok?'Run reported complete':'Run needs attention'} — ${when(run.completedAt)}`));
  if(run.note){const details=el('details','');details.append(el('summary','','Technical details'),el('pre','picks-run-technical',run.note));host.append(details);}
}
...
add('dashboard_picks_capture','Save one original source pick or unresolved caption to the private dashboard. Never invent odds, selection, event, or date. Unknown fields empty, odds null. accountId names which of the creator’s verified accounts it came from; leave it empty when unknown. Reopen the pick’s exact source link once immediately before calling this, then pass that same link as checkedUrl and the time as checkedAt; a save without them is refused. Firm, lean, or unclear is derived from the captured wording, never chosen by you or by Preston. transcriptId ties the pick to a stored reel transcript; the selection must appear in that transcript. Historical/uncertain timing must use capturedBeforeStart=false. Places no bet.',{type:'object',properties:{...properties,accountId:{type:'string'},transcriptId:{type:'string'},checkedUrl:{type:'string'},checkedAt:{type:'string'}},required:[...Object.keys(properties),'checkedUrl','checkedAt'],additionalProperties:false},async p=>{const {checkedUrl,checkedAt,...pick}=p;
  const derived=classify(pick.originalText||'',pick.selection||'');
...
let timingSignature='';
if(typeof setInterval==='function')setInterval(()=>{if(loaded&&!busy){const signature=desk.picks.filter(p=>currentPick(p)).map(p=>p.id).join('|');if(signature!==timingSignature){timingSignature=signature;renderClean();renderRunStatus();renderList();}}},1000);
window.PitchfordPicks={init,open:()=>{if(loaded)render();},read:async()=>{if(!await load())throw new Error('Unlock Sports Picks or check the connection and try again.');return desk;},stats,groupPicks,currentPick,cleanList,leanList,verify:confirmPick,plainLine,classify,sources:()=>SOURCES,extractFromTranscript,transcribers:TRANSCRIBERS};
})();
