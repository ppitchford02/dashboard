...
function renderRunStatus(){const host=$('picks-run-status'),run=desk.latestRun;if(!run){host.className='receipt error';host.textContent='No verified automated Sports Picks run has reported yet.';return;}const completed=when(run.completedAt),status=run.outcome==='complete'||run.outcome==='no_work'?'Verified run':'Run needs attention';host.className='receipt'+(status==='Verified run'?'':' error');host.textContent=`${status}: ${run.accountsChecked} accounts checked, ${run.picksSaved} picks saved, ${run.accountsBlocked} blocked — ${completed}.${run.note?' '+run.note:''}`;}
